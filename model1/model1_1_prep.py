...
def model1_1_fbf_var(filePath,frame_rate,bit_rate):
    ...
    ans=[np.mean(qp_list),frame_rate, i_intra_flicker, np.mean(qp_max_list),np.mean(qp_min_list),bit_rate, np.mean(frame_i_sts),np.mean(skip_ratio), np.mean([my_max(mv0[i],mv1[i]) for i,a in enumerate(mv0)]),kfr,width[0]*height[0]]
...
def batch():
    ...
    for n in range(1,71):
        ...
        l=(model1_1_fbf_var(csv_filepath+'\\'+str(n)+'.csv',frame_rate,bit_rate))
        #writer.writerow(l,)
        xdata.append(l);
    #csvFile.close()
    return xdata
